...
class RoundRobinScheduler:
    ...
    def _get_scraper(self, domain: str) -> cloudscraper.CloudScraper:
        ...
        if domain in self.scraper_pool:
            if self.scraper_request_counts[domain] >= self.session_rotation_interval:
                print(
                    f"[Scheduler] Rotating scraper for {domain} after {self.scraper_request_counts[domain]} requests"
                )
        ...
        if domain not in self.scraper_pool:
            ...
            self.scraper_pool[domain] = cloudscraper.create_scraper(
    ...
    def _calculate_delay(self, config: DomainConfig) -> float:
        ...
        if config.success_count > 0:
            failure_rate = config.failure_count / (
    ...
    def fetch_with_retry(
        self, url: str, headers: Optional[Dict] = None, method: str = "GET", **kwargs
    ) -> Optional[Any]:
        """
            ...
            try:
                ...
                if response.status_code == 200:
                    if (
                        ...
                        print(
                            f"[Scheduler] Possible block detected on {domain}, retrying..."
                        )
                    ...
                    print(
                        f"[Scheduler] Server error {response.status_code} from {domain}"
                    )
                    config.failure_count += 1
                ...
                print(
                    f"[Scheduler] Error fetching {url} (attempt {attempt + 1}/{config.max_retries}): {e}"
                )
                config.failure_count += 1
    ...
    def crawl_with_round_robin(
        ...
        print(
            f"[Scheduler] Crawling {len(urls)} URLs across {len(domain_queues)} domains"
        )
            ...
            for domain in active_domains:
                ...
                if response and response.status_code == 200:
                    try:
                        ...
                        completed += 1
                        print(
                            f"[Scheduler] ✓ Success ({completed} completed, {failed} failed)"
                        )
                    except Exception as e:
                    ...
                    failed += 1
                    print(
                        f"[Scheduler] ✗ Failed ({completed} completed, {failed} failed)"
                    )
    ...
    def _print_stats(self):
        ...
        for domain, config in self.domains.items():
            ...
            print(
                f"  {domain}: {config.success_count}/{total} successful ({success_rate:.1f}%)"
            )
    ...
    def configure_domain(
        ...
        print(
            f"[Scheduler] Configured {domain}: delay={config.min_delay}-{config.max_delay}s, "
...
class EnhancedCrawler:
    ...
    def crawl_pages(
        ...
        self.scheduler.configure_domain(
            domain, min_delay=min_delay, max_delay=max_delay
        )
        ...
        for page_num in range(start_page, start_page + pages):
            ...
            while attempts < max_attempts:
                ...
                try:
                    ...
                    print(
                        f"[EnhancedCrawler] ✓ Page {page_num} complete: {len(page_companies)} companies"
                    )
                    break
                    ...
                    if attempts >= max_attempts:
                        print(
                            f"[EnhancedCrawler] Skipping page {page_num} after {max_attempts} failures"
                        )
                        break
