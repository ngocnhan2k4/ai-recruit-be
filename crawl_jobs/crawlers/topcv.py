    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 403:
                print(f"[!] 403 Forbidden on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
            
            if resp.status_code == 429:
                print(f"[!] Rate limited (429) on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
            
            if resp.status_code != 200:
                print(f"[!] HTTP {resp.status_code} on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
            
            # Check for Cloudflare challenge
            if "Just a moment..." in resp.text:
                print(f"[!] Cloudflare challenge on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
            
            if "Attention Required" in resp.text:
                print(f"[!] Cloudflare attention on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
                
            # Check for captcha
            if "captcha" in resp.text.lower() or "hcaptcha" in resp.text.lower():
                print(f"[!] Captcha detected on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
            
            # Check for valid HTML content
            if "<html" not in resp.text.lower():
                print(f"[!] Invalid HTML on {url}, retrying ({attempt + 1}/{max_retries})...")
                continue
...
def scrape_job_detail(scraper, card, base_url: str, link: str, companies: dict, headers: dict):
    ...
    resp_text = fetch_with_retry(scraper, job_url, headers=detail_headers, max_retries=3, base_delay=3)
    
    if not resp_text:
        print(f"    ⚠️ Failed to fetch job detail page")
    ...
    if company_link and company_link.get("href"):
        ...
        comp_text = fetch_with_retry(scraper, company_url, headers=comp_headers, max_retries=2, base_delay=3)
...
def scrape_page(scraper, page_num, headers, max_jobs_per_page=None):
    ...
    html = fetch_with_retry(scraper, listing_url, headers=listing_headers, max_retries=5, base_delay=5)
    if not html:
    ...
    for idx, card in enumerate(job_cards):
        try:
            ...
            if not link_elem:
                continue
            link = link_elem.get("href", "").replace("?ta_source=ITJobs_LinkDetail", "", 1)
    ...
    for page_num in range(start_page, start_page + pages):
        ...
        while attempts < 3:
            try:
                ...
                page_companies = scrape_page(scraper, page_num, headers, max_jobs_per_page)
                break
            except Exception as e:
