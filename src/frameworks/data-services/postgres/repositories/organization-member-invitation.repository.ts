...
import { organizationMemberInvitations, users } from "../models";
import { Inject, Injectable } from "@nestjs/common";
import { type DBDrizzle } from "@/frameworks/data-services/postgres/types";
import { GeneralQuery, PaginatedResult } from "@/common/types/api";
import { ilike, or, and, lt, isNull, desc } from "drizzle-orm";
  ...
  implements IOrganizationMemberInvitationRepository
{
  ...
  > {
    ...
    if (query.cursor) {
      whereConditions.push(
        lt(organizationMemberInvitations.createdAt, new Date(query.cursor)),
      );
    }
